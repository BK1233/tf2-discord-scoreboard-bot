<#
  ...
  - Creates a Dockerized Discord bot that reads TF2 scoreboard JSON on Windows
  - Generates the folder structure and docker-compose.yml for the bot
  - Installs Python dependencies in a virtual environment

  Modify the parameters below to suit your installation before running.
#>
...
services:
  tf2scorebot:
    ...
    volumes:
      - "{0}:/data/scoreboard.json:ro"
'@ -f $ScoreFile | Out-File -Encoding UTF8 -FilePath (Join-Path $ProjectDir "docker-compose.yml")
...
'@ | Out-File -Encoding UTF8 -FilePath (Join-Path $ProjectDir ".env.example")

Write-Host "==> Project files created at $ProjectDir" -ForegroundColor Green
Write-Host "Update the volume path in docker-compose.yml if necessary." -ForegroundColor Yellow
